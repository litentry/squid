...
export default class Typegen extends Command {
  ...
  static examples = [`$ devkit typegen balances`];
  ...
  private getChains = () => {
    const typgenFiles = readdirSync(
      `${this.getProjectRootDir()}/prawns/${this.prawn}/typegen/`,
      { withFileTypes: true }
    );
  ...
  private runTypeGen = (chain: string) =>
    execSync(
      `yarn squid-substrate-typegen prawns/${this.prawn}/typegen/${chain}Typegen.json`,
