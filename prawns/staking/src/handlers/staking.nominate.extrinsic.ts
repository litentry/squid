...
import { decodeAddress, getOrCreate, getRegistry, encodeAddress } from '../utils';
...
export default (network: SubstrateNetwork, tokenIndex: number) =>
  async (ctx: CallHandlerContext<Store>) => {
    ...
    filteredTargets.forEach((target) => {
      if (!target) { return; }
      saveNewValidatorAccount(ctx, encodeAddress(network, target), symbol, network);
    });

};


async function saveNewValidatorAccount(ctx: CallHandlerContext<Store>, target: string, symbol: string, network: SubstrateNetwork) {
